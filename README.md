...
`se` is a command-line utility similar to [`sed`]. It is a tool meant for searching or replacing
text in files or outputs of other programs piped into it. `se` can be called as
...
While it has near feature parity with `sed`, there are some differences in syntax, behavior of some
commands, or their naming. `se` is more of a re-design of `sed` than a re-implementation.
...
* `s/src/dst/[limit]` – use regular expression to replace `src` with `dst` in the pattern space.
  ...
  Instead of `/`, other character can be used as a delimiter, for example `s#src#dst#`.
  `s^src$dst$` is the whole line match form for substituting using the `^src$` regular expression.
...
* `j`, `J` – push content of the hold space at the back of the pattern space
  using a newline character as separator. With `J` do the same as above,
  but without the separator.
* `k`, `K` – push content of the pattern space at the back of hold space
  using a newline character as separator. It is a shortcut for `xjx`.
  With `K` do the same as above, but without the separator.
