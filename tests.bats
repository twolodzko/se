...
@test "Hold and get work" {
   run bats_pipe --returned-status=1 printf "ok\nwrong\n" \| ./se '1h.gP'
   [ "$status" -eq 0 ]
   [ "$output" = "ok" ]
}
...
@test "Eval command from pattern space" {
   run diff <(ls -la) <(./se 'eP' <<< 'ls -la')
   [ "$status" -eq 0 ]
}

@test "Eval command from argument" {
   run diff <(ls -la) <(./se 'e"ls -la" Pq' README.md)
   [ "$status" -eq 0 ]
}
...
@test "Be like yes with branching" {
   run diff <(yes "hi!" | head -n 100) <(echo | ./se ':x p"hi!" b x' | head -n 100)
   [ "$status" -eq 0 ]
}

@test "Branch with many jumps" {
   run diff <(yes "yes" | head -n 100) <(echo | ./se ':r by; !:y P"y"be; !:e P"e"bs; !:s P"s\n" br' | head -n 100)
   [ "$status" -eq 0 ]
}
...
@test "Delete command" {
   run diff <(sed '/the/d' README.md) <(./se -a '/the/d' README.md)
   [ "$status" -eq 0 ]
}
...
@test "Handle broken pipes" {
	run bats_pipe --returned-status=1 yes hello \| ./se -a p \| head -n 1
   [[ ! "$output" =~ "Broken pipe" ]]
   [ "$status" -eq 0 ]
}
